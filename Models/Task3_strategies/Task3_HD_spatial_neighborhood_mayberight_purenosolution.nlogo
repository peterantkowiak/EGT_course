...
patches-own [strategy_current strategy_new fitness strat_P_cur strat_P_new change_prob fitness_pure]
...
to setup
    ...
    ask patches [set fitness_pure baseline_fitness]
...
to go
  ...
  if strategy = "Pure" [set fitness_pure baseline_fitness set strategy_current strategy_new]
  if strategy = "Mixed" [ ask patches [set fitness baseline_fitness] ]
  if strategy = "Mixed" [ask patches [set strategy_current strategy_new]]

   if strategy = "Mixed" [ask patches [set strat_P_cur strat_P_new]]
   if strategy = "Mixed" [ask patches [mutate]]
...
to reproduce_pure ; patch
 ...
 ; let competitor one-of neighbors
  set change_prob ([fitness_pure] of competitor - fitness_pure) / (benefit)
  if change_prob > 0 [
